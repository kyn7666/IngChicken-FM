#!/bin/bash
# Submit rollout evaluation for ER-only CL checkpoints (all 4 LIBERO suites).
#
# Usage:
#   EVAL_ALL=1 bash submit_er_eval.sh
#   EVAL_TASK=5 bash submit_er_eval.sh
#   BENCHMARKS="object spatial" EVAL_ALL=1 bash submit_er_eval.sh
...
echo "Submitting ER eval  [tag=${BATCH_TAG}, tasks=${JOB_SUFFIX}]"
...
for BENCH in ${BENCHMARKS}; do
  CONFIG="${BASE}/configs/cl_${BENCH}_er.yaml"
  if [[ ! -f "${CONFIG}" ]]; then
    echo "Config not found, skipping: ${CONFIG}" >&2
    continue
  fi

  LOG_PREFIX="${LOG_DIR}/er_eval_${BENCH}_${BATCH_TAG}"

  sbatch <<EOF
#!/bin/bash
#SBATCH --job-name=fm_eval_${BENCH}
...
exec singularity exec --nv --writable-tmpfs \\
  ...
  bash -lc '
    ...
    python -m scripts.eval_sequential \\
      --config /workspace/configs/cl_${BENCH}_er.yaml \\
      ${EVAL_ARG}
  '
EOF
