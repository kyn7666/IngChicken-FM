#!/bin/bash
# Submit ER-only baseline jobs for all 4 LIBERO suites.
#
# Usage:
#   bash submit_er_baselines.sh
#   BENCHMARKS="object spatial" bash submit_er_baselines.sh
...
echo "Submitting ER-only baselines  [tag=${BATCH_TAG}]"
...
for BENCH in ${BENCHMARKS}; do
  ...
  LOG_PREFIX="${LOG_DIR}/er_${BENCH}_${BATCH_TAG}"

  sbatch <<EOF
#!/bin/bash
#SBATCH --job-name=fm_er_${BENCH}
